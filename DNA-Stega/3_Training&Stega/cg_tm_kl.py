...
def txt_process_sc_FromKl1(lines, len_sc, divied):
    ALL = []
    num1 = divied
    temp_out = ''
    for line in lines:
        ...
        # 将序列按照每行 len_sc 的长度进行分割
        for i in range(0, len(temp), len_sc):
            chunk = temp[i:i + len_sc]
            if len(chunk) == len_sc:
                temp1 = ''
                # 将每行按照 num1 进行分组
                for j in range(0, len(chunk), num1):
                    temp1 += chunk[j:j + num1] + ' '
                ALL.append(temp1.strip())
            else:
                # 如果最后一行长度不足，可以选择是否补齐或丢弃
                pass
    return ALL, temp_out
