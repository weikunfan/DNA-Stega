...
def pxy(line,sigle,two_base):
	...
	print("Pxy计算完成，部分结果:", out[:5])  # 显示部分结果样本
...
class Corpus(object):
	...
	def _build_corpus(self):
		...
		print(f"语料库构建完成，共包含 {self.sentence_num} 句。")
		print(f"最长句长: {self.max_sentence_length}, 最短句长: {self.min_sentence_length}")
...
def split_corpus(data_path, train_path, test_path, max_len=200, min_len=5, ratio=0.8, seed=0, encoding='utf8'):
	...
	print(f"数据集分割完成，训练集样本数: {len(train)}，测试集样本数: {len(test)}")
...
class Generator(object):
	...
	def _padding(self, batch_data):
		...
		print(f"批数据已填充至统一长度: {max_length}")
	...
	def build_generator(self, batch_size, shuffle=True, padding=False):
		...
		while True:
			...
			# print(f"生成批次，大小为: {len(batch_data)}")
			yield batch_data
