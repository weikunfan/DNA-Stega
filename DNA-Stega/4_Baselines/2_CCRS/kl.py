...
def pxy(line, sigle, two_base):
    ...
    for x in BaseX:
        for y in BaseY:
            ...
            DicTwoBaseNum[bases] = 0  # 初始化所有组合为0

    # 初始化单核苷酸计数为0
    for base in BaseX:
        DicSingleBaseNum[base] = 0

    # 更新单核苷酸计数
    for base, num in sigle:
        if base in BaseX:  # 只处理ATCG
            DicSingleBaseNum[base] = num

    # 更新二核苷酸计数
    for bases, num in two_base:
        if len(bases) == 2 and bases[0] in BaseX and bases[1] in BaseX:  # 验证有效的二核苷酸
    ...
    for bases in BaseMartix:
        ...
        SecondBase = bases[1]
        # 避免除以0
        denominator = DicSingleBaseNum[FirstBase] * DicSingleBaseNum[SecondBase]
        if denominator == 0:
            DicP[bases] = 0  # 当分母为0时返回0
        else:
            PXY = (DicTwoBaseNum[bases] * len(line)) / denominator
            DicP[bases] = PXY
